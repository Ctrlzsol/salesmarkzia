...
/** Convert a number of days into an Arabic human-readable duration string.
 *  E.g. 400 -> "سنة وشهر وآحادان"
 */
function calcDuration(days: number): string {
  if (!days || days <= 0) return "0 يوم";
  ...
  if (rem    > 0 || parts.length === 0) parts.push(`${rem || days} يوم`);
  return parts.join(" و");
}
          ...
          {TABS.map((tab, idx) => {
            ...
            return (
              <motion.button key={tab.id}
                ...
                transition={{ delay: idx * 0.055, type: "spring", stiffness: 260, damping: 22 }}
                whileHover={{ scale: 1.025, x: -3 }}
                whileTap={{ scale: 0.97 }}
                onClick={() => { setActiveTab(tab.id); setSidebarOpen(false); }}
                className={`w-full flex items-center gap-3.5 px-3.5 py-2.5 rounded-xl text-right transition-colors duration-200 border-r-2
                  ${active 
                ...
                <motion.div
                  ...
                  animate={{ boxShadow: active ? `0 0 14px ${tab.color}55` : "0 0 0px transparent" }}
                ...
                <AnimatePresence>
                  {active && (
                    <motion.div
                      ...
                      className="mr-auto w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse"
                    />
                  )}
                </AnimatePresence>
              </motion.button>
